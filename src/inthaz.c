 ...
 *  adapted from pgamma by J.K. Lindsey, January, 1998
 *
 ...
 *
 *    #include "Mathlib.h"
 *    double ihgamma(double x, double a, double scale);
 *
 *  DESCRIPTION
 *
 *    This function computes the integrated hazard function for the
...
double ihgamma(double x, double p, double scale)
{
	...
	return -log(1-pnorm(pn1, 0.0, 1.0));
    }

    /* if x is extremely large compared to p then return xbig */
...
double ihlogis(double x, double location, double scale)
{
#ifdef IEEE_754
	...
	return log(1.0 + exp((x - location) / scale));
}
